...
const JSON_SCHEMA = JSON.stringify(schemas_1.schemas);
...
let wasWordBasedSuggestionsEnabled = null;
let wasCopilotEnabled = null;
...
function getYamlSameLevelProperties(document, position) {
    const text = document.getText();
    const lines = text.split("\n");
    let properties = [];
    let currentIndent = position.character;
...
async function activate(context) {
    ...
    if (vscodeYaml) {
        ...
        yamlExtensionAPI.registerContributor(SCHEME, (resource) => {
    ...
    function addEntrySuggestion(name, description, completionItems) {
        const item = new vscode.CompletionItem(name, vscode.CompletionItemKind.Property);
        item.detail = description;
        item.insertText = `${name}:\n  `;
        completionItems.push(item);
    }
    function addTextSuggestion(name, description, completionItems) {
        const item = new vscode.CompletionItem(name, vscode.CompletionItemKind.Text);
        item.detail = description;
        item.insertText = `${name}`;
        completionItems.push(item);
    }
    const provider = vscode.languages.registerCompletionItemProvider({ language: 'yaml' }, {
        provideCompletionItems(document, position, token, context) {
            ...
            //   - name: "Entry"
            //     description: "Description"
            function addNestedSuggestions(properties, parentPath, currentPath) {
                ...
                Object.keys(properties).forEach((key) => {
                    ...
                    if (subProperties) {
                        ...
                        if (ref) {
                            ...
                            // @ts-ignore
                            markdownDescription = schemas_1.schemas.$defs[refKey]?.markdownDescription ? schemas_1.schemas.$defs[refKey].markdownDescription : markdownDescription;
                        }
            ...
            function suggestPropertiesForPath(properties, path) {
                if (path.length === 0) {
                ...
                function addSuggestionAvoidDuplicate(key) {
                    let markdownDescription = "New Entry.";
                    let newKey = key;
                    // @ts-ignore
                    const ref = properties[key]?.$ref;
            ...
            suggestPropertiesForPath(schemas_1.schemas.properties, keyPath);
    ...
    schemas_1.schemas.$defs.bukkit_entity_type.enum.forEach(element => {
    ...
    schemas_1.schemas.$defs.bukkit_materials.enum.forEach(element => {
    ...
    function updateDecorations() {
        ...
        decorateEnums(doc, text, activeEditor, "Vanilla material", schemas_1.schemas.$defs.bukkit_materials.enum, typesDecos.materials);
        decorateEnums(doc, text, activeEditor, "Vanilla entity type", schemas_1.schemas.$defs.bukkit_entity_type.enum, typesDecos.entities);
    ...
    async function handleDocumentRefresh(document) {
...
async function setCopilot(val) {
    // Check if it exists
    const copilotEnable = vscode.workspace.getConfiguration('github.copilot').get('enable');
    if (copilotEnable) {
    ...
    if (val) {
        ...
        if (!neverWarnAboutCopilot) {
            const result = await vscode.window.showInformationMessage('[!] GitHub Copilot temporarily disabled for ItemsAdder YAML.\nCopilot causes only issues and suggestions are not correct. Please use the CTRL+SPACE shortcut to allow ia-vscode to provide proper autocomplete suggestions.', "OK", "Never Show Again");
 ...
 */
function handleForcedDiagnostics(doc, text, activeEditor, diagnostics, diagnosticsArr) {
    diagnostics.clear();
    diagnosticsArr = [];
        ...
        node.items.forEach((pair) => {
            ...
            if (hasTexture && hasTextures) {
                if (resourceNode.range) {
                    ...
                    const diagnostic = new vscode.Diagnostic(new vscode.Range(startPos, endPos), "Use either `texture` or `textures` property, not both.", vscode.DiagnosticSeverity.Error);
            ...
            if (hasModelPath && (hasTexture || hasTextures)) {
                if (resourceNode.range) {
                    const startPos = activeEditor.document.positionAt(resourceNode.range[0]);
                    const endPos = activeEditor.document.positionAt(resourceNode.range[1]);
                    const diagnostic = new vscode.Diagnostic(new vscode.Range(startPos, endPos), "Do not use `texture` or `textures` property when `model_path` is specified.", vscode.DiagnosticSeverity.Error);
                    diagnosticsArr.push(diagnostic);
                }
            }
            ...
            if (!resourceNode.has("material") && !armorNode) {
                if (resourceNode.range) {
                    const startPos = activeEditor.document.positionAt(resourceNode.range[0]);
                    const endPos = activeEditor.document.positionAt(resourceNode.range[1]);
                    const diagnostic = new vscode.Diagnostic(new vscode.Range(startPos, endPos), "Missing `material` property!", vscode.DiagnosticSeverity.Error);
                    diagnosticsArr.push(diagnostic);
                }
            }
        });
...
function decorateGenericTexts(activeEditor, description, regEx, decorationType) {
...
function decorateEnums(doc, text, activeEditor, description, enumsSchema, decos) {
...
function decorateBackgroundTextBlocks(activeEditor, description, textToMatch, decorationType) {
    const appliedDecorations = [];
    const text = activeEditor.document.getText();
...
const prevDecorationsAlternated = [];
function decorateBackgroundItemsAlternated(activeEditor) {
    if (!activeEditor) {
        return;
    }
